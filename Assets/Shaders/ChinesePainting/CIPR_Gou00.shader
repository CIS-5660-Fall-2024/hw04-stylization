// Chinese Ink Painting Render Gou
Shader "CIPR/CIPR_Gou00"
{
Properties {
	//Color
	...
	//Terrain
	_TerrainTexture ("Terrain Map", 2D) = "white" {}
...
SubShader {
	...
	Pass {
		...
		fixed4 frag (v2f i) : SV_Target {
			...
			fixed3 terrainFactor_y = tex2D(_TerrainTexture, TRANSFORM_TEX(i.worldPos.zx, _TerrainTexture));
			fixed3 terrainFactor_z = tex2D(_TerrainTexture, TRANSFORM_TEX(i.worldPos.xy, _TerrainTexture));
			fixed3 terrainFactor_x = tex2D(_TerrainTexture, TRANSFORM_TEX(i.worldPos.yz, _TerrainTexture));
			fixed3 terrainFactor = terrainFactor_y * abs(i.normal.y);
			terrainFactor = lerp(terrainFactor.rgb, terrainFactor_z, abs(i.normal.z));
			terrainFactor = lerp(terrainFactor.rgb, terrainFactor_x, abs(i.normal.x));
	...
	Pass
	{
		...
		fixed4 frag (v2f i) : SV_Target {
			...
			fixed4 col = tex2D(_ColorRemap, i.mcrmd.z) * pow(_OutlineColor, 1.0/2.2)* 2;
	...
	Pass
	{
		Tags { "RenderType"="Opaque" "LightMode"="UniversalForwardOnly" }
		Cull Front
		...
		fixed4 frag (v2f i) : SV_Target {
			fixed4 col = tex2D(_ColorRemap, i.rmUV) * pow(_OutlineColor, 1.0/2.2) * 2;
