...
void PickColor_float(float3 Highlight, float3 Midtone, float3 Shadow, float Diffuse, float Threshold1, float Threshold2, out float3 OUT)
{
...
void CheckShadow_float(float ShadowTex, float Shadow, out float OUT)
{
    if(Shadow > 0.5f)
    {
        OUT = Shadow;
